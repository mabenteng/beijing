...
class Sql(object):
    ...
    def __init__(self,tb="wx_jobs"):#thtjobs
        ...
        if is_trinity() and True: # 更改线上就是false
            self.db = pymysql.connect(host='localhost',user='root',password='root',database='tht',charset='utf8')
        else:
            # 连接投行通内网数据库
            # self.db = pymysql.connect(host='10.168.187.8',user='root000',password='root123',database='regulation',charset='utf8')
            # 连接投行通公网ip
            # print("投行通")
            self.db = pymysql.connect(host='111.206.168.10',user='root999999',password='root123',database='regulation',charset='utf8')
        self.table=tb
...
if __name__ == '__main__':
    #测试是否是e14电脑
    print(is_trinity())
    sql="select * from wx_gm_user where id =1"
    k=Sql("wx_exam")
    data=k.query(sql)
    print(data)
    pass
